...
TerrainMaterial::TerrainMaterial(glm::vec3 pDiffuseColor):_diffuseColor (pDiffuseColor)
{
...
void TerrainMaterial::_lazyInitializeShader() {
    ...
    if (!_shader) {
        ...
        glUniform1i(_shader->getUniformLocation("specularTexture"), 6);
    }
}
...
void TerrainMaterial::render(World* pWorld, Mesh* pMesh, const glm::mat4& pModelMatrix, const glm::mat4& pViewMatrix, const glm::mat4& pProjectionMatrix) {
    ...
    glUniform1f(_shader->getUniformLocation("timeNow"), (float)clock()/1000);
    glUniform1f(_shader->getUniformLocation("timeScale"), 2.0f);
    ...
    {
        for (GLint i = 0; i < (GLint)_diffuseTextures.size(); ++i)
        {
            //setup texture slot 0
            glActiveTexture(GL_TEXTURE0 + 2 + i);
            //bind the texture to the current active slot
            glBindTexture(GL_TEXTURE_2D, _diffuseTextures[i]->getId());
        }
    }

    if (_specularTexture)
    {
        glActiveTexture(GL_TEXTURE0 + 2 + (GLint)_diffuseTextures.size() + 1);
...
void TerrainMaterial::CacheLightUniformLocations()
{
