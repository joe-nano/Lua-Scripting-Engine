...
void GameObject::LookAt(glm::vec3 targetPosition)
{
    glm::vec3 localPosition = getLocalPosition();

    glm::vec3 worldUp   = glm::normalize(glm::vec3(0.0f, 1.0f, 0.0f));
    glm::vec3 forward   = glm::normalize((localPosition - targetPosition));
    glm::vec3 right     = glm::cross(worldUp, forward);
    glm::vec3 up        = glm::cross(forward, right);

    glm::mat4 objectToWorld;

    objectToWorld[0][0] = right.x;
    objectToWorld[0][1] = right.y;
    objectToWorld[0][2] = right.z;
    objectToWorld[1][0] = up.x;
    objectToWorld[1][1] = up.y;
    objectToWorld[1][2] = up.z;
    objectToWorld[2][0] = forward.x;
    objectToWorld[2][1] = forward.y;
    objectToWorld[2][2] = forward.z;

    objectToWorld[3][0] = localPosition.x;
    objectToWorld[3][1] = localPosition.y;
    objectToWorld[3][2] = localPosition.z;
    objectToWorld[3][3] = 1;

    setTransform(objectToWorld);
}
